...
| 하이퍼파라미터 | 설정값 | 상세 설명 |
| :--- | :--- | :--- |
| **Objective / Metric** | `mae` | 태양광 일사량 예측의 오차를 최소화하기 위해 평균 절대 오차(MAE) 채택 |
| **n_estimators** | `10,000` | 조기 종료(early_stopping)와 결합하여 충분한 학습 횟수 확보 |
| **learning_rate** | `0.01` | 낮은 학습률을 적용하여 모델의 안정적인 수렴 유도 |
| **num_leaves** | `127` | 풍부한 시차(Lag) 변수의 복잡한 패턴을 학습하기 위해 높은 복잡도 설정 |
| **subsample** | `0.8` | 각 트리 학습 시 데이터의 80%만 활용하여 모델의 과적합(Overfitting) 방지 |
| **colsample_bytree** | `0.7` | 학습 시 피처의 70%만 무작위 선택하여 모델의 일반화 성능 강화 |
| **min_data_in_leaf** | `50` | 개별 리프 노드에 최소 데이터 수를 보장하여 노이즈에 대한 민감도 감소 |
| **random_state** | `42` | 실험 결과의 재현성(Reproducibility) 확보를 위한 고정 시드 |
...
* **최종 성과:** 초기 단순 모델 대비 성능을 향상시켰으며, 최종적으로 **MAE 50점 초반대MAE**를 달성했습니다.
